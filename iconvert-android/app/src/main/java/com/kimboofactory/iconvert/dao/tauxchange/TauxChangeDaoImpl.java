...
public class TauxChangeDaoImpl extends GenericDao
    ...
    @Override
    public long save(List<RateData> tauxChanges) {
        int count = 0;
        final SQLiteDatabase db = getDb();
        final String whereClause = IConvertContract.TauxChange.COL_CODE + "=?";

        db.beginTransaction();
        for (RateData tauxChange : tauxChanges) {
            ...
            value.put(IConvertContract.TauxChange.COL_TAUX_CHANGE, tauxChange.getValue());
    ...
    public void save(JSONObject tauxChangeJson) {
        ...
        final List<RateData> tauxChanges = new ArrayList<>();
    ...
    @Override
    public RateData getByCode(final String code) {
    ...
    private RateData cursorToTauxChange(final Cursor c) {
        RateData tauxChange = null;
        if(c.moveToFirst()) {
            tauxChange = null;
                    //new RateData(c.getLong(0), c.getString(1), c.getDouble(2));
        }
        return tauxChange;
    }
